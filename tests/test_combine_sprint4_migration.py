"""Migration smoke test for combine sprint 4 (commenting)."""
...
SPRINT4_REVISION = "e9b3f7d2a1c8"
PARENT_REVISION = "d8a1c5b9f2e4"
...
def test_combine_sprint4_migration_round_trip() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        db_path = Path(tmp) / "test_combine_sprint4.db"

        up = _alembic(["upgrade", SPRINT4_REVISION], db_path)
        ...
        # Sprint 3 tables must still be there.
        assert "combine_parser_jobs" in _table_names(db_path)

        up2 = _alembic(["upgrade", SPRINT4_REVISION], db_path)

"""Migration smoke test for combine sprint 3 (parser jobs / results)."""
...
NEW_TABLES = {"combine_parser_jobs", "combine_parser_results"}
SPRINT3_REVISION = "d8a1c5b9f2e4"
PARENT_REVISION = "c2f9a8b6d4e7"
...
def test_combine_sprint3_migration_round_trip() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        db_path = Path(tmp) / "test_combine_sprint3.db"

        up = _alembic(["upgrade", SPRINT3_REVISION], db_path)
        ...
        # Sprint 2 tables must still be there.
        assert "combine_warming_jobs" in _table_names(db_path)

        up2 = _alembic(["upgrade", SPRINT3_REVISION], db_path)
